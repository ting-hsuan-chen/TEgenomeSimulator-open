...
#Load params_chr from YAML file config_genome.yml in same directory
def parse_random_genome_yaml():
    params_chr = yaml.load(open('config_random_genome.yml', 'r'), Loader=yaml.FullLoader)
    return params_chr

#Load params_chr from YAML file config_custum_genome.yml in same directory (function created by THC)
def parse_custom_genome_yaml():
    params_chr = yaml.load(open('config_custom_genome.yml', 'r'), Loader=yaml.FullLoader)
    return params_chr
...
def load_repeats_chr(params_chr):
    ...
    with open(params_chr['rep_list'], 'r') as repeats_file:
        ...
        for line in repeats_file:
            ...
            sd = int(elem[5])
            indels = int(elem[6])
            tsd = [int(list(elem[7])[0]), int(list(elem[7])[2])]
            frag = int(elem[9])
            nest = int(elem[10])
...
def assign_chr_coord_repeats(params_chr, repeats_dict):
    #Total number of repeats (i.e. insertions)
    total_num_rep = sum([repeats_dict[rep].num_rep for rep in repeats_dict])
    #Total genome bp
    ...
    chr_cumm_dict = dict(zip(chr_id, chr_len_cumm))
    ...
    for num in random_coords:
        for key, value in chr_cumm_dict.items():
            if num <= value:
                random_repeats_coords.append((key, num))
                break
    return random_repeats_coords
...
def generate_genome_sequence(repeats_dict, rand_rep_pos, rand_chr_dict, shuffled_repeats):
    ...
    for chromosome in rand_chr_dict.keys():
        ...
        rand_rep_pos_filter = [coord for chr_id, coord in rand_rep_pos if chromosome in chr_id]
        ...
        #Increment the start of the slice for next chromosome
...
#Print final genome sequence to stdout (function created by THC)
def print_genome_data(genome_dict, new_repeats_coord_dict):
    #Setup output directory    
    Path("./result/sim_random_genome").mkdir(parents=True, exist_ok=True)
    os.chdir("./result/sim_random_genome")
    
    #Create genome fasta file
    fasta_out = open("genome_out_sequence.fasta", "w")
    for chromosome in genome_dict.keys():
        seq = genome_dict[chromosome]
    ...
    fasta_rep = open("genome_out_repeats.fasta", "w")
    gff_rep = open("genome_out_repeats.gff", "w")
    counts = 1
...
def main():
    ##For genome with multiple chromosomes
    #Load parameters
    params_chr = parse_random_genome_yaml()
    #Set seed
    ...
    #Insert repeats into chr sequences (after applying identity changes), report new chr sequences and coordinates
    genome, new_repeats_coord = generate_genome_sequence(repeats_dict, repeats_coord, chrs_dict, shuffled_repeats)
    #Output fasta file with new chr sequences, repeats, and GFF.
    print_genome_data(genome, new_repeats_coord)
