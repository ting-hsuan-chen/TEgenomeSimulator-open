...
from random_genome_seq_TEs import parse_random_genome_yaml, load_repeats_chr, generate_mismatches, add_indels, add_base_changes
from random_genome_seq_TEs import get_identity, create_TSD, fragment
...
def generate_genome_nests(repeats, isrt_te_dict, gff, genome):
    ...
    #Output fasta files of the genome, repeats and the gff file
    print_genome_nest_data(genome, isrt_te_dict, nest_te_dict, new_gff)
...
def print_genome_nest_data(genome, isrt_te_dict, nest_te_dict, new_gff):
    ...
    fasta_out = open("genome_out_sequence_nest.fasta", "w")
    ...
    te_fa_out = open("genome_out_repeats_nest.fasta", "w")
    ...
    gff_out = open("genome_out_repeats_nest.gff", "w")
    for i in new_gff:
        gff_out.write("\t".join(map(str,i)) + "\n")
    gff_out.close()
...
def main():
    ...
    ##For genome with multiple chromosoms
    ##Load parameters
    ...
    gff_file = "./result/sim_random_genome/genome_out_repeats.gff"
    fasta_file = "./result/sim_random_genome/genome_out_sequence.fasta"
    isrt_te_fasta = "./result/sim_random_genome/genome_out_repeats.fasta"
    ...
    generate_genome_nests(repeats_dict, isrt_te, gff, genome)
