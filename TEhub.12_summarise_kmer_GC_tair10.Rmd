---
title: "Compare kmer and GC profile of the simulated and original genomes"
output: 
  pdf_document:
...
#range <- '5_500'
genomename <- 'tair10'
SIM <- paste0("/workspace/cflthc/scratch/2022_Actinidia_TE/10.01_TEgenomeSimulator_output/sim_tair10_genome/")
...
oriGC <- read.table("/workspace/cflthc/scratch/2022_Actinidia_TE/10.01_TEgenomeSimulator_output/GC_analysis/tair10_ori/tair10_ori_GC.txt", header = F, skip = 1, sep = "")
simGC <- read.table("/workspace/cflthc/scratch/2022_Actinidia_TE/10.01_TEgenomeSimulator_output/GC_analysis/tair10_sim/tair10_sim_GC.txt", header = F, skip = 1, sep = "")
oriK <- read.table("/workspace/cflthc/scratch/2022_Actinidia_TE/10.01_TEgenomeSimulator_output/kmer_analysis/tair10_ori/tair10_ori_histo_31m.out", header = F, sep = " ")
simK <- read.table("/workspace/cflthc/scratch/2022_Actinidia_TE/10.01_TEgenomeSimulator_output/kmer_analysis/tair10_sim/kmer_histo.out", header = F, sep = " ")
colnames(oriGC) <- c("chr", "length", "GC")
colnames(simGC) <- c("chr", "length", "GC")
colnames(oriK) <- c("kmer_depth", "distinct_kmers")
colnames(simK) <- c("kmer_depth", "distinct_kmers")
```

```{r, prep_GC, include=FALSE}
oriGC <- mutate(oriGC, genome = "original")
simGC <- mutate(simGC, genome = "simulated")
GC <- rbind(oriGC, simGC)
```

```{r, fig21_plot_length, echo=FALSE, fig.width = 6, fig.asp = 0.6}
plotwidth <- 6
plotheight <- 3.6
options(repr.plot.width=plotwidth, repr.plot.height=plotheight)
plot <- ggplot(GC, aes(x = chr, y = length, fill = genome)) +
  geom_bar(stat="identity", position=position_dodge()) +
  scale_fill_manual(values = c("black", "grey50"), name = "Genome") +
  ggtitle(paste0("Chromosome length comparison\n", genomename))  + 
  xlab("Chromosomes") + ylab("Length (bp)") +
  geom_text(aes(label = paste0(round(length / 1000000, 2), "\nMb")), hjust = 0.5, vjust = -0.25, size=rel(3), position = position_dodge(width = .9)) +
  ylim(0, (max(GC$length)) * 1.2) +
  theme(plot.title = element_text(size = rel(1), hjust=0),
        axis.text.y = element_text(size = rel(1.2)),
        axis.text.x = element_text(size = rel(1.2), angle = 45, vjust = 1, hjust=1),
        axis.title.x = element_text(size = rel(1)),
        axis.title.y = element_text(size = rel(1)),
        panel.background = element_rect(fill = "white", color = "black"))
plot
```
```{r, save_fig21, echo=FALSE}
png(filename = paste0(OUT, "021_compare_chr_length_", genomename, ".png"), width = plotwidth, height = plotheight, units = "in", res = 1200)
print(plot)
while (!is.null(dev.list()))  dev.off()
```


```{r, fig22_plot_GC, echo=FALSE, fig.width = 6, fig.asp = 0.6}
plotwidth <- 6
plotheight <- 3.6
options(repr.plot.width=plotwidth, repr.plot.height=plotheight)
plot <- ggplot(GC, aes(x = chr, y = GC, fill = genome)) +
  geom_bar(stat="identity", position=position_dodge()) +
  scale_fill_manual(values = c("black", "grey50"), name = "Genome") +
  ggtitle(paste0("Chromosome GC % comparison\n", genomename))  + 
  xlab("Chromosomes") + ylab("GC content (%)") +
  geom_text(aes(label = GC), hjust = 0.5, vjust = -0.25, size=rel(3), position = position_dodge(width = .9)) +
  ylim(0, (max(GC$GC)) * 1.2) +
  theme(plot.title = element_text(size = rel(1), hjust=0),
        axis.text.y = element_text(size = rel(1.2)),
        axis.text.x = element_text(size = rel(1.2), angle = 45, vjust = 1, hjust=1),
        axis.title.x = element_text(size = rel(1)),
...
plot
```
```{r, save_fig22, echo=FALSE}
png(filename = paste0(OUT, "022_compare_chr_length_", genomename, ".png"), width = plotwidth, height = plotheight, units = "in", res = 1200)
print(plot)
while (!is.null(dev.list()))  dev.off()
```

```{r, compare_idn_distribution, include=FALSE}
#simulated
gff <- '/workspace/cflthc/scratch/2022_Actinidia_TE/10.01_TEgenomeSimulator_output/sim_tair10_genome/tair10_repeat_annotation_out_nest.gff'
...
sim_idn_hist <- breakdown |> select(TE_rename, Identity) |> mutate(label = "simulated")
ori_idn_hist <- RMout |> select(TE_rename, Identity) |> mutate(label = "original")
idn_hist <- rbind(sim_idn_hist, ori_idn_hist)
```

```{r, echo=FALSE, fig23, fig.width = 12, fig.asp = .67}
...
plot <- ggplot(idn_hist, aes(as.numeric(Identity), fill = label)) +
    #call geom_histogram with position="dodge" to offset the bars and manual binwidth of 2
  geom_histogram(alpha=0.5, binwidth = 0.02, position="identity") +
    #geom_histogram(position = "stack", binwidth = 0.02, color = "white") +
    scale_fill_manual(values=c("dodgerblue4", "gold2")) +
    ggtitle("Simulated vs original tair10 genome") +
    xlab("Identity") + ylab("TE loci") +
...
```{r, save_fig23, echo=FALSE}
png(filename = paste0(OUT, "023_compare_idn_distribution", genomename, ".png"), width = plotwidth, height = plotheight, units = "in", res = 1200)
print(plot)
while (!is.null(dev.list()))  dev.off()
```
...
```{r, prep_kmer, include=FALSE}
oriK <- mutate(oriK, genome = "original")
simK <- mutate(simK, genome = "simulated")
Kmer <- rbind(oriK, simK)
```
...
```{r, fig22_plot_kmer, echo=FALSE, fig.width = 12, fig.asp = 0.5}
plotwidth <- 12
plotheight <- 6
options(repr.plot.width=plotwidth, repr.plot.height=plotheight)

plot <- ggplot(Kmer |> filter(kmer_depth <= 200), aes(x = kmer_depth, y = distinct_kmers)) +
    #call geom_histogram with position="dodge" to offset the bars and manual binwidth of 2
  geom_line() +
...
data.sum <- te_by_superfamily_sim_genome_tair(tegff)
```
...
plot <- plot_sim_identity_by_loci_tair(breakdown)
plot
```
...
plot <- plot_sim_integrity_by_loci_tair(breakdown)
plot
```
