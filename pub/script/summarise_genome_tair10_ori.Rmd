---
title: "A summary of the TE landscape of the original tair10 genome"
...
setwd('../../')
WRK <- getwd()
SIM <- file.path(WRK, "pub/output/tair10/output/TEgenomeSimulator_tair10_run7_result/")
...
rmtbl <- paste0(SIM, "repeatmasker/GCF_000001735.4_TAIR10.1_genomic.fna.tbl")
```
...
```{r, fig9, echo=FALSE, fig.width = 6, fig.asp = .67, out.width="60%"}
plotwidth <- 6
plotheight <- 4
options(repr.plot.width=plotwidth, repr.plot.height=plotheight)
ymax <- 1750
...
ymax <- 3000
ybreak <- 500
plot <- plot_ori_integrity_by_loci(rmout |> filter(itg >= 0), ymax, ybreak, genomename)
plot
```
...
```{r, fig11, echo=FALSE, fig.width = 8, fig.asp = .5}
plotwidth <- 8
plotheight <- 4
options(repr.plot.width=plotwidth, repr.plot.height=plotheight)

plot <- plot_ori_integrity_by_loci(rmout |> filter(itg >= 0) |> filter(family == 'ATIS112A'), 50, 10, genomename)
plot
```

```{r, save_fig11, echo=FALSE}
png(filename = paste0(OUT, "te_loci_integrity_", genomename, "ATIS112A.png"), width = plotwidth, height = plotheight, units = "in", res = 1200)
print(plot)
while (!is.null(dev.list()))  dev.off()
```